...
class MAPSNet(ResNet50):
    def __init__(self, nclass, BatchNorm=nn.BatchNorm2d, aux=False, sobel=False, constrain=False, n_input=3, **kwargs):
        ...
        self.myUpsamplelayer3 = self._make_layer_ups(BasicBlock_ups, 32, 1,
                                                     stride=2)
        ...
        self.conv_trans = nn.Conv2d(224, 64, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        out11 = self.myUpsamplelayer3(out22)

        myout3 = torch.abs(out33 - out3)
        myout2 = torch.abs(out22 - out2)
        myout1 = torch.abs(out11 - out1)

        # cat连接
        myout3 = F.interpolate(myout3, size=(64, 64), mode='bilinear', align_corners=False)
        myout2 = F.interpolate(myout2, size=(64, 64), mode='bilinear', align_corners=False)
        myout1 = F.interpolate(myout1, size=(64, 64), mode='bilinear', align_corners=False)
        myout = torch.cat((myout3, myout2, myout1), dim=1)

        # # 选择设备
        # device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        # myout = myout.to(device)  # 将输入数据移动到设备上

        # 输入转换
        myout = self.conv_trans(myout)
        myout = F.interpolate(myout, size=(128, 128), mode='bilinear', align_corners=False)
